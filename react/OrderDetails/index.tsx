  ...
  public render() {
    ...
    if (!order.hasOwnProperty('error')) {
      ...
      if (order.shippingData.address.addressType == 'pickup') {
        displayShippingInformation = (
          <div className={`${styles.shippingInformationHolder}`}>
            ...
            <span>{order.shippingData.address.street},{' '}{order.shippingData.address.number}</span>
            <span>{order.shippingData.address.city}, {order.shippingData.address.state}, {order.shippingData.address.postalCode}</span>
          </div>
        )
      }
      ...
      if (order.clientProfileData.isCorporate) {
        displayCorporateInformation = (
          <div className={`${styles.corporateInformationHolder}`}>
            <span className={`${styles.bold}`}>
              {formatMessage({id: messages.company.id})}: {order.clientProfileData.corporateName}
            </span>
            <span className={`${styles.bold}`}>
              {formatMessage({id: messages.tradeRegister.id})}: {order.clientProfileData.corporateDocument}
            </span>
          </div>
        )
      }
      ...
      if (order.packageAttachment.packages && order.packageAttachment.packages.length) {
        ...
        if (packageItem.courierStatus && packageItem.courierStatus.data.length) {
      ...
      let tagColor = order.status === settings.constants.invoiced ? 'blue' : 'green';
      ...
      if (order.status === settings.constants.window_to_cancel) {
              ...
              <div className={`mr4 w-30`}>
                ...
                >
                  <div className={`${styles.height6}`}>
                    <h3 className={`mt0 mb6`}>{formatMessage({id: messages.automatic.id})}</h3>
                  </div>
                </SelectableCard>
              </div>
                    ...
                    >
                      <div className={`${styles.height6}`}>
                        ...
                        <p className={`gray ma0 ${styles.fontSmall}`}>
                          {formatMessage({id: messages.rate.id})}{': '} {item.rateAmount} {item.rateCurrency}
                        </p>
                        <p className={`gray ma0 ${styles.fontSmall}`}>
                          {formatMessage({id: messages.rateVat.id})}{': '} {item.rateVatAmount} {item.rateCurrency}
                        </p>
                        <p className={`gray ma0 ${styles.fontSmall}`}>
                          {formatMessage({id: messages.service.id})}{': '} {item.service}
                        </p>
                        <p className={`gray ma0 ${styles.fontSmall}`}>
                          {formatMessage({id: messages.deliveryDays.id})}{': '} {item.deliveryDays}
                        </p>
                      </div>
                    </SelectableCard>
                  </div>
        ...
        if (this.state.numberOfParcels > 1) {
          ...
          const color = setWeight - totalWeight === 0 ?
          ...
          weightDifference = (
            <div className={`mt6 pa3 br2 dib mr5 mv0 ba ${color} ${styles.height25}`}>
                ...
                <div className={`mb4 mt4 flex flex-row ${styles.flex1}`}>
                  ...
                  <div className={`ml4 mt7 flex flex-column ${styles.flex05}`}>
                    <Checkbox
                      ...
                      id="option-0"
                      label={formatMessage({id: messages.recipientSupport.id})}
        ...
        tagColor = 'orange';
        extraStatusMessage = ` - ${formatMessage({id: messages.requestAwb.id})}`
      ...
      return (
        <div className={`pa6 ${styles.flex05}`}>
          <Box title={`${formatMessage({id: messages.order.id})}#${order.orderId}`}>
                              ...
                              <div className={`${styles.productInfo}  ${styles.font14}`}>
                                {productsSpecs[item.productId] ? (
                                  <Collapsible
                                    ...
                                    onClick={e => this.collapse(item.productId, e.target.isOpen)}
                                    isOpen={collapsibles.hasOwnProperty(item.productId) ? collapsibles[item.productId] : false}
                                    caretColor="primary"
                                  >
                                    {productsSpecs[item.productId]}
                                  </Collapsible>
            ...
            <div className={`mb4 mt4 ${styles.textCenter}`}>{displayTrackingNumber}</div>
            <div className={`mb4 mt4 ${styles.textCenter}`}>{displayTrackinUrl}</div>
          </Box>
            ...
            bottomBar={
              <div className="nowrap">
                <span className={`mr4`}>
                  ...
                  >{formatMessage({id: messages.close.id})}</Button>
                </span>
              </div>
            }
          >
